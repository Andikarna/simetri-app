...
@section('title', 'Detail Daftar Potong Tembaga')
...
@section('content')
    <div class="container-fluid p-5" style="background: linear-gradient(to right, #eef2f3, #8e9eab);">
        ...
        <div class="container bg-white shadow-lg rounded p-3 mb-4 d-flex justify-content-between align-items-center"
            ...
            <a href="{{ route('cutting.print', $request->id) }}" target="_blank" class="btn btn-primary rounded-pill px-4 py-2">
        ...
        <div class="container bg-white shadow-lg rounded p-4" style="border-radius: 12px;">
            ...
            @php
                ...
                $cuttingRecords = \App\Models\CuttingRecord::where('copper_cutting_id', $request->id)
                    ->get()
                    ->groupBy('ukuran');
                ...
                $cuttingSummary = $cuttingRecords->map(function ($items) {
                    return [
                        ...
                        'total_cut_length' =>  4000 - $items->sum('quantity'),
                    ];
                });
            @endphp
            <!-- Cutting Summary Table -->
            <div class="mt-5">
                <h5 class="fw-bold">Total Pemotongan dari Cutting Record</h5>
                <table class="table table-bordered text-center shadow-sm">
                    ...
                    <tbody>
                        @if (!empty($cuttingSummary) && count($cuttingSummary))
                        ...
                        @else
                            <tr>
                                <td colspan="3" class="text-center text-muted">Belum ada data pemotongan</td>
                            </tr>
                        @endif
                    </tbody>
                </table>
